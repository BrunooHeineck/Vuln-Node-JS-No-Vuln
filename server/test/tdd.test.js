...
describe('Test Driven Development', () => {
	...
	describe('RENDER', () => {
		test('Deve renderizar na tela "Usuário ou senha inválidos" ao tentar login e senha estiver incorreta.', async () => {
			const response = await request(endpoints.loginerr);

			expect(response.data).toContain('Usuário ou senha inválidos');
		});

		test('Deve renderizar na tela "Usuário ou senha inválidos" ao tentar login e usuário não existir.', async () => {
